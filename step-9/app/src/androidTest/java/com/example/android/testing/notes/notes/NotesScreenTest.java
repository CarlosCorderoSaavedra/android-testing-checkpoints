...
public class NotesScreenTest {
    ...
    String descriptionTest = "There are many legitimate cases when you cannot determine R.id a";
    ...
    public void addNoteToNotesList() throws Exception {
        ...
        onView(withId(R.id.add_note_description))
                .perform(typeText(descriptionTest));

        // Click on the OK button to finish the note
        onView(withId(R.id.fab_add_notes))
                .perform(click());
        ...
        onView(withId(R.id.note_detail_title))
                .check(matches(withText(title)))
                .perform(click());
        // Check the android image in the note appear
        onView(withId(R.id.note_detail_image))
                .check(matches(isDisplayed()));
